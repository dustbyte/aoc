...
fn main() {
    ...
    // wild assumption that symbols with only 2 adjacent parts are gears but it works
    let sum: u32 = symbols.values().filter(|v| v.len() == 2).map(|vals| vals.iter().product::<u32>()).sum();
