use std::{fs, process::exit, collections::{HashSet, HashMap}, char};
...
fn get_seeds(line: &str) -> Vec<u64> {
    line.split_once(":").unwrap().1.split_whitespace().map(|n| n.parse::<u64>().unwrap()).collect()
}
...
fn main() {
    ...
    for seed in seeds {
        ...
        //println!("seed: {}, score = {}", seed, this_score);
